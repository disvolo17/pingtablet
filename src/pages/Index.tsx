...
const Index = () => {
  ...
  return (
    <>
      {/* HERO */}
      <section className="relative overflow-hidden border-b" style={{borderColor: "var(--glass-border)"}}>
        <div className="container relative pt-6 pb-5 md:pt-20 md:pb-16">
          ...
          >
            <h1 className="font-display text-[72px] md:text-[120px] leading-[0.92] tracking-[1px]" style={{color: "hsl(var(--ink))"}}>
              ПИНГ<br />
              <em className="not-italic" style={{color: "hsl(var(--orange))"}}>ТАБЛЕТ</em>
            </h1>
            <p className="mt-3 text-[13px] md:text-base leading-relaxed max-w-xs" style={{color: "hsl(var(--subtle))"}}>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2.5 md:gap-3 max-w-sm">
              <Link to="/leaderboard" className="rounded-2xl p-3.5 border relative overflow-hidden" style={{background: "#161616", borderColor: "#e8572a"}}>
                ...
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[10px] font-medium uppercase tracking-[0.8px]" style={{color: "hsl(var(--subtle))"}}>Live</span>
                </div>
              ...
              <Link to="/events" className="col-span-2 rounded-2xl p-3.5 border flex items-center justify-between" style={{background: "#161616", borderColor: "rgba(255,255,255,0.07)"}}>
                ...
                <ArrowRight className="h-4 w-4 shrink-0" style={{color: "hsl(var(--orange))"}} />
              </Link>
            </div>
          </motion.div>
        </div>
      ...
      {/* HOW */}
      <section className="container py-4 border-b" style={{borderColor: "var(--glass-border)"}}>
        <div className="flex items-center justify-center gap-2 text-[12px]" style={{color: "hsl(var(--subtle))"}}>
          {[
      ...
      <section className="container pt-section">
        <div className="flex items-end justify-between mb-4">
          <h2 className="text-[18px] font-semibold" style={{color: "hsl(var(--ink))"}}>Сейчас играют</h2>
          <Link to="/leaderboard" className="text-[12px] flex items-center gap-1" style={{color: "hsl(var(--orange))"}}>Рейтинг <ArrowRight className="h-3 w-3" /></Link>
        </div>
        {loading ? (
              ...
              <Link key={t.id} to={`/t/${t.id}`} className="rounded-2xl p-4 border flex items-center justify-between group transition-opacity hover:opacity-80" style={{background: "#161616", borderColor: "rgba(255,255,255,0.07)"}}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-2">
                    {t.status === "live" ? (
                      <><span className="h-1.5 w-1.5 rounded-full animate-pulse" style={{background: "hsl(var(--orange))"}} /><span className="text-[11px] font-medium" style={{color: "hsl(var(--orange))"}}>В эфире</span></>
                    ) : (
                  ...
                  <h3 className="text-[14px] font-medium truncate" style={{color: "hsl(var(--ink))"}}>{t.name}</h3>
                  {t.location && <p className="text-[11px] mt-0.5 truncate" style={{color: "hsl(var(--subtle))"}}>{t.location}</p>}
                </div>
                <ArrowRight className="h-4 w-4 ml-3 shrink-0" style={{color: "hsl(var(--orange))"}} />
              </Link>
            ))}
          </div>
        )}
